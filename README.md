...
面向零基础英语学习者的 AI 个性化学习应用：AI 按需生成单词卡（音标/释义/对话体语境例句/讲解）→ 闪卡学习 + FSRS 间隔重复复习 → 产出型测验 → 学过的词 AI 编成故事阅读。全程发音（edge-tts 微软神经语音）。

## 功能

- **今日队列**：新词 + 按 FSRS 排期的复习卡，每日自动安排
- **闪卡学习**：翻面看释义 / 例句 / **AI 生成的两句对话语境**（聊天气泡样）
- **评分**：忘了 / 模糊 / 记得 / 太简单 → FSRS 决定下次复习时间
- **词毕业**：复习 3 次后例句自动变成句子卡继续学
- **测验**：中译英×3 + 选词 + 填空，服务端判分（忽略大小写/标点）
- **单词本**：全部学过的词，一格一排，点击发音
- **故事模式**：AI 用你学过的词编一篇英文小故事，点词看释义、评分、发音
- **加词**：输入想学的词（逗号分隔），AI 生成完整学习卡片
- **学习统计**：今日已学、累计词汇、🔥连续学习天数（streak）
...
> 首次使用：进「加词」页输入几个单词（如 `apple, banana, teacher`），AI 生成词卡后明天开始进入队列。也可以立刻点「开始今天的学习」复习已有的到期卡。

## 测试

```bash
python -m pytest                 # 65 个后端测试
...
| GET | /api/stats | 学习统计 + streak |
| POST | /api/reviews | 评分提交（FSRS 调度） |
| GET | /api/quiz / POST /api/quiz/score | 测验 / 判分 |
| POST | /api/typing/check | 打字判分 |
| GET | /api/cards / POST /api/cards/generate | 单词本 / AI 生成词卡 |
...
```
app/
  models/       SQLAlchemy 模型（Card / Review / Story / StoryWord）
  routers/      API 路由（today / reviews / quiz / cards / stories / tts / health）
  services/     业务层（词卡生成 / FSRS 调度 / 毕业 / 故事生成 / 判分 / 模型路由）
